...
function submitScore(playerName, score) {
	fetch('http://your-server-ip:3000/submit', {
		method: 'POST',
		headers: {
...
function fetchHighScores() {
	fetch('http://your-server-ip:3000/getHighScores')
...
function displayHighScores(data) {
	...
	data.forEach((item, index) => {
		...
		entry.innerHTML = `${index + 1}. ${item.playerName} - ${item.score}`;
...
var sgypGame = {
	...
	resetFlex: function() {
		...
		for (var i = 0; i < this.Heng; i++) {
			...
			// If the game state is 0 (game over)
			if (this.state == 0) {
